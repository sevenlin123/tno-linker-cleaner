...
# v0.4: testing: project the coordinate to low latitude (not active) 
# v0.4.1: added some comments
##########################################################################
...
#transfrom topographic to barycentric    
#calculate the real topographic position slow everything down and use a lot if memory,
#but the accuracy improvement is negligible
def topo_to_bary(tri_pos):
    ...
    ts = load.timescale()
    t = ts.tai(jd=mjd+2400000.500428) #37 leap seconds
    earth = planets['earth']
        
    #ICRS xyz of earth. to cacluate actual topo position, unmark the following line 
    ...
    #below are the transformation of projection reference
    #It works correctly, but not comparable with current linker code 'connect.py'
